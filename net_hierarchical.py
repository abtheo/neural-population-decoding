    ...
    def __init__(self, P):
        ...
        for r_o in range(self.s_os):
            for c_o in range(self.s_os):
                self.layer_i[r_o, c_o] = WTALayer(
                    self.w, self.h, 2, P.ms)
        ...
        if not self.load_weights_ih(P.pf_pretrained_weights_ih):
            self.weights_ih = self.initialize_weights(
                (self.s_os, self.s_os, self.K_h, self.w, self.h, 2))
        # If weights are not loaded ...
        if not self.load_weights_ho(P.pf_pretrained_weights_ho):
            self.weights_ho = self.initialize_weights(
                (self.K_o, self.s_os, self.s_os, self.K_h))

        if self.topdown_enabled:
            self.weights_oh = self.initialize_weights(
                (self.K_h, self.s_os, self.s_os, self.K_o))
        ...
        for r_o in range(self.s_os):
            for c_o in range(self.s_os):
                self.inhibitors_h[r_o, c_o] = Inhibitor(
                    P.hertz_o, inh_start_h, P.inh_change_h, P.inh_ratio_h, P.ms)

        self.ts_last_spike = np.zeros((self.s_os, self.s_os), dtype=np.int16)
        self.t_last_spike = 0

        # assume the 784 here is derrived from MNIST 28*28,
        # so replace with 176*128 ?
        const = 176*128  # old: 784
        self.e_min_h = -self.log_c * const * self.hertz_o * 0.2 * \
            0.0001 if is_none(P.e_min_h) else P.e_min_h  # ***
        self.e_min_o = -self.log_c * const * self.hertz_o * \
            0.2 * 0.0001 if is_none(P.e_min_o) else P.e_min_o
...
# =============================================================================
    ...
    def load_weights_ih(self, pf_pretrained_weights_ih):
        ...
        if str(weights_ih) != "nopath":
            self.weights_ih = np.zeros(
                (self.s_os, self.s_os, self.K_h, self.w, self.h, 2))
            for r_o in range(self.s_os):
                for c_o in range(self.s_os):
                    self.weights_ih[r_o, c_o] = weights_ih[r_o*self.s_os+c_o]
            print(
                f"Succesfully loaded  input-hidden weights from <{pf_pretrained_weights_ih}>.")
            return True
        return False
    ...
    def load_weights_ho(self, pf_pretrained_weights_ho):
        ...
        if str(weights_ho) != "nopath":
            ...
            print(
                f"Succesfully loaded hidden-output weights from <{pf_pretrained_weights_ho}>.")
            return True
        return False
    ...
    def propagate_input(self, spike_times, t):
        ...
        for r_o in range(self.s_os):
            for c_o in range(self.s_os):
                ...
                self.layer_i[r_o, c_o].fire_multiple(
                    spike_times[r_o, c_o, :, :, :, t], t)

                # Compute the excitation and inhibition and update membrane potentials accordingly
                excitation = ((self.weights_ih[r_o, c_o]+self.log_c)
                              * spike_times[r_o, c_o, :, :, :, t]).sum((1, 2, 3))
                inhibition = 0 if self.softmax_probabilities else self.inhibitors_h[r_o, c_o].get_control(
                    spike_times[r_o, c_o, :, :, :, t].sum())
    ...
    def propagate_hidden(self, t, r_o, c_o, learn=True):
        ...
        ks_h, n_spikes_h = self.get_spike_indices(
            self.layer_h[r_o, c_o].excitation[0, 0], self.K_h, self.hertz_h)

        if not self.softmax_probabilities:  # Adapt inhibition according to spiking activity
            self.inhibitors_h[r_o, c_o].update(
                t, t-self.ts_last_spike[r_o, c_o], n_spikes_h > 0)
        ...
        for k_h in ks_h:
            ...
            if learn:  # Update the incoming connections weights of neuron <k_h>
                self.stdp(self.layer_i[r_o, c_o], self.layer_h[r_o,
                          c_o], self.weights_ih[r_o, c_o, k_h], t, k_h)
                if self.topdown_enabled:
                    self.stdp(
                        self.layer_o, self.layer_h[r_o, c_o], self.weights_oh[k_h, r_o, c_o], t, k_h, self.topdown_stdp_mult)
    ...
    def propagate_output(self, t, learn=True):
        ...
        ks_o, n_spikes_o = self.get_spike_indices(
            self.layer_o.excitation[0, 0], self.K_o, self.hertz_o)
        ...
        for k_o in ks_o:
            ...
            if self.topdown_enabled:
                ...
                if not isinstance(self.td_curve, type(None)):
                    curve_index = min(
                        self.n_spikes_since_reset_o[k_o]-1, len(self.td_curve)-1)
    ...
    def propagate(self, spike_times, t, learn_h=True, learn_o=True):
        ...
        if not self.softmax_probabilities:  # Include inhibition and limit the membrane potential of neurons
            ...
            self.layer_o.excitation = np.clip(
...
# =============================================================================
    ...
    def hidden_to_pixels(self, index_circuit):
        ...
        weights_black = self.weights_ih[index_circuit[0],
                                        index_circuit[1], :, :, :, 0] + self.log_c
        weights_white = self.weights_ih[index_circuit[0],
                                        index_circuit[1], :, :, :, 1] + self.log_c
        ...
        weight_pixels = ((weights+max_value)/(max_value*2)
                         * 255).astype(np.uint8)
    ...
    def out_to_pixels(self):
        ...
        pixels = np.zeros((self.K_o, 128, 176))
        for r_o in range(self.s_os):
            for c_o in range(self.s_os):
                ...
                for k_o in range(self.K_o):
                    ...
                    n_highest = w_sorted[w_sorted >
                                         w_sorted[0] - show_range].size
                    indices = indices[:n_highest]
                    highest_weights = self.weights_ho[k_o,
                                                      r_o, c_o, :][indices]
                    pixel = 0
                    for ind, w in zip(indices, highest_weights):
                        pixel += (self.log_c + w) * pixels_hidden[ind]
                    pixel /= n_highest
                    pixels[k_o, r_o*self.w:r_o*self.w+self.w,
                           c_o*self.h:c_o*self.h+self.h] += pixel

        # Normalize the weights, then scale them to pixel values (0-255)
        max_value = np.max(np.abs(pixels))
        weight_pixels = ((pixels+max_value)/(max_value*2)
                         * 255).astype(np.uint8)
    ...
    def print_plot_save(self, data_handler, X, labels, index_im_all, size_dataset, P, time_start):
        ...
        if len(self.ls_n_spikes) > P.print_interval:
            ...
            if index_im_all % P.print_interval == 0 and index_im_all > 0:
                print(f"avg spikes over last {P.print_interval}:", np.mean(
                    self.ls_n_spikes))
                print(f"std spikes over last {P.print_interval}:", np.std(
                    self.ls_n_spikes))
                print(
                    f"Min spikes: {np.min(self.ls_n_spikes)}, Max spikes: {np.max(self.ls_n_spikes)}")
...
# =============================================================================
        ...
        if P.plot_weight_ims_h and (index_im_all % P.plot_interval == 0 or
            ...
            indices = [k for k in range(self.K_h)]
            weight_pixels = self.hidden_to_pixels((r_o, c_o))
            input_segment = X[index_im_all, r_o*self.w:r_o *
                              self.w+self.w, c_o*self.h:c_o*self.h+self.h]
        ...
        if P.plot_weight_ims_o and (index_im_all % P.plot_interval == 0 or
                                    index_im_all == size_dataset-1):
            # Determine which neurons to plot
            indices = [k for k in range(self.K_o)]
            ...
            for r_o in range(self.s_os):
                for c_o in range(self.s_os):
                    for k_h, p in enumerate(self.hidden_to_pixels((r_o, c_o))):
                        pf_weight_im_h = P.pd_weight_ims_h + \
                            P.nft_weight_ims_h.format(
                                index_im_all, r_o, c_o, k_h)
        ...
        if P.save_weight_ims_o and (index_im_all % P.save_weight_ims_interval == 0 or
            ...
            for k_o, p in enumerate(self.out_to_pixels()):
                pf_weight_im_o = P.pd_weight_ims_o + \
                    P.nft_weight_ims_o.format(index_im_all, k_o)
        ...
        if P.save_weights_h and (index_im_all > 0 and index_im_all % P.save_weights_interval == 0 or
            ...
            if len(os.listdir(P.pd_weights_h)) > 0:  # If there are already stored weights
                # Create a directory for past weights
                if len(os.listdir(P.pd_weights_h)) == self.s_os**2:
                    os.mkdir(P.pd_weights_h+self.nd_weights_past)
                # Move past weights to separate directory
                for nf in os.listdir(P.pd_weights_h):
                    if os.path.isfile(P.pd_weights_h+nf):  # Only move (weight) files
                        shutil.move(P.pd_weights_h+nf,
                                    P.pd_weights_h+self.nd_weights_past+nf)

            # Save the weights of the input-hidden connections
            for r_o in range(self.s_os):
                for c_o in range(self.s_os):
                    pf_weights_h = P.pd_weights_h + \
                        P.nft_weights_h.format(index_im_all, r_o, c_o)
        ...
        if P.save_weights_o and (index_im_all > 0 and index_im_all % P.save_weights_interval == 0 or
            ...
            if len(os.listdir(P.pd_weights_o)) > 0:  # If there are already stored weights
                ...
                # Move past weights to separate directory
                for nf in os.listdir(P.pd_weights_o):
                    if os.path.isfile(P.pd_weights_o+nf):  # Only move (weight) files
                        shutil.move(P.pd_weights_o+nf,
                                    P.pd_weights_o+self.nd_weights_past+nf)

            # Save the weights of the hidden-output connections
            pf_weights_o = P.pd_weights_o + \
                P.nft_weights_o.format(index_im_all)
    ...
    def load_test_results(run_id, pdt_test_results="[01LF] results/run-hierarchical_{}/test_results/", nf_test_parameters="test_parameters.pkl"):
        ...
        weights_ho = np.load(
            pd_test_results+test_parameters.nf_test_weights_ho)
        weights_ih = np.load(
            pd_test_results+test_parameters.nf_test_weights_ih)
        test_labels = np.load(pd_test_results+test_parameters.nf_test_labels)
        neuron_label_counts = np.load(
            pd_test_results+test_parameters.nf_neuron_label_counts)
        neuron_image_counts = np.load(
            pd_test_results+test_parameters.nf_neuron_image_counts)
