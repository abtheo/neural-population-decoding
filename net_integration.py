...
class NetworkIntegration(Network):
    ...
    def __init__(self, P, P_a, P_b, P_c):
        ...
        self.e_min_o = -self.log_c * 176*128 * self.hertz_o * \
    ...
    def load_weights_f(self, pf_pretrained_weights):
        ...
        if str(weights) != "nopath":
            ...
            print(
                f"Succesfully loaded input-output weights from <{pf_pretrained_weights}>.")
            return True
        return False
    ...
    def propagate(self, spike_times_a, spike_times_b, spike_times_c, t, learn_h=True, learn_o=True):
        ...
        if self.topdown_enabled:
            if self.net_a.learn_o:
                for k_o_a in ks_o_a:
                    self.net_a.stdp(self.layer, self.net_a.layer_o,
                                    self.weights_td[0, k_o_a], t, k_o_a, self.topdown_stdp_mult)
        ...
        for k_o_a in ks_o_a:
            self.layer.excitation[0,
                                  0] += self.weights[0, :, k_o_a] + self.log_c
        for k_o_b in ks_o_b:
            self.layer.excitation[0,
        ...
        ks, n_spikes = self.get_spike_indices(
            self.layer.excitation[0, 0], self.K, self.hertz_o)
        ...
        for k in ks:
            ...
            if self.topdown_enabled:  # Send feedback to the hierarchical output layer
                if not self.net_a.t_last_spike == t:
                    ...
                    inhibition = 0 if self.softmax_probabilities else self.net_a.inhibitor.get_control(
                        1)
                    self.net_a.layer_o.excitation[0,
                                                  0] += excitation - inhibition
                    self.net_a.layer_o.excitation = np.clip(
                        self.net_a.layer_o.excitation, 0, -self.net_a.e_min_o)
                if not self.net_b.t_last_spike == t:
                    ...
                    if not isinstance(self.td_curve, type(None)):
                        curve_index = min(
                            self.n_spikes_since_reset[k]-1, len(self.td_curve)-1)
                        excitation = excitation * self.td_curve[curve_index]
                    inhibition = 0 if self.softmax_probabilities else self.net_b.inhibitor.get_control(
                        1)
                    self.net_b.layer_o.excitation[0,
                                                  0] += excitation - inhibition
    ...
    def train(self, data_handler, P):
        ...
        X_a, labels, _, _ = data_handler.load_mnist(
            tag_mnist=self.net_a.tag_mnist)
        ...
        for ith_slice in range(0, util.SHAPE_MNIST_TRAIN[0]//data_handler.s_slice):
            ...
            spike_data_a = data_handler.get_mnist_spikes(
                ith_slice, tag_mnist=self.net_a.tag_mnist, train=True)
            spike_data_b = data_handler.get_mnist_spikes(
                ith_slice, tag_mnist=self.net_b.tag_mnist, train=True)
            for index_im, (spike_times_a, spike_times_b) in enumerate(zip(spike_data_a, spike_data_b)):
                ...
                spike_times_a = self.tile_input(
                    spike_times_a, self.net_a.s_os, self.net_a.s_is)
                spike_times_b = self.tile_input(
                    spike_times_b, self.net_b.s_os, self.net_b.s_is)
                ...
                self.net_a.print_plot_save(
                    data_handler, X_a, labels, index_im_all, util.SHAPE_MNIST_TRAIN[0], self.P_a, time_start)
                self.net_b.print_plot_save(
                    data_handler, X_b, labels, index_im_all, util.SHAPE_MNIST_TRAIN[0], self.P_b, time_start)
                time_start = self.print_plot_save(
                    data_handler, X_a, X_b, labels, index_im_all, util.SHAPE_MNIST_TRAIN[0], P, time_start)

                # Reset spike counters
                self.net_a.n_spikes_since_reset_h = np.zeros(
                    (self.net_a.s_os, self.net_a.s_os, self.net_a.K_h), dtype=np.uint16)
                self.net_a.n_spikes_since_reset_o = np.zeros(
                    self.net_a.K_o, dtype=np.uint16)
                self.net_b.n_spikes_since_reset_h = np.zeros(
                    (self.net_b.s_os, self.net_b.s_os, self.net_b.K_h), dtype=np.uint16)
                self.net_b.n_spikes_since_reset_o = np.zeros(
                    self.net_b.K_o, dtype=np.uint16)
                self.n_spikes_since_reset = np.zeros(self.K, dtype=np.uint16)
...
# =============================================================================
    def test(self, data_handler, P):
        ...
        _, _, X_a, labels = data_handler.load_mnist(
            tag_mnist=self.net_a.tag_mnist)
        ...
        for ith_slice in range(0, util.SHAPE_MNIST_TEST[0]//data_handler.s_slice):
            ...
            # Retrieve slice <ith_slice> of the spike data
            spike_data_a = data_handler.get_mnist_spikes(
                ith_slice, tag_mnist=self.net_a.tag_mnist, train=False)
            spike_data_b = data_handler.get_mnist_spikes(
                ith_slice, tag_mnist=self.net_b.tag_mnist, train=False)
            for index_im, (spike_times_a, spike_times_b) in enumerate(zip(spike_data_a, spike_data_b)):
                ...
                self.net_b.print_plot_save(
                    data_handler, X_b, labels, index_im_all, util.SHAPE_MNIST_TRAIN[0], self.P_b, time_start)
                time_start = self.print_plot_save(
                    data_handler, X_a, X_b, labels, index_im_all, util.SHAPE_MNIST_TEST[0], P, time_start)
...
# =============================================================================
    ...
    def print_plot_save(self, data_handler, X_a, X_b, labels, index_im_all, size_dataset, P, time_start):
        ...
        if P.plot_weight_ims and (index_im_all % P.plot_interval == 0 or
            ...
            indices = [k for k in range(self.K)]
        ...
        if P.save_weight_ims and (index_im_all % P.save_weight_ims_interval == 0 or
            ...
            for k, p in enumerate(self.weights_to_pixels()):
                pf_image = P.pd_weight_ims + \
                    P.nft_weight_ims.format(index_im_all, k)
