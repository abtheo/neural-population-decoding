...
def run_hierarchical(train=True):
    ...
    #     with open(f'./data/spikes/X_test_spikes_{i}.pkl', 'wb') as f:
    ...
    for ith_slice in tqdm(range(0, 30)):
        ...
        for index_im, spike_times in enumerate(spike_data):
            ...
            #     data_handler, X_train, labels_train, index_im_all, SHAPE_MNIST_TRAIN[0], P, time_start)
    ...
    neuron_label_counts = np.zeros((network.K_o, 10), dtype=np.uint32)

    # For each image the number of times each neuron spiked
    neuron_image_counts = np.zeros((SHAPE_MNIST_TEST[0], network.K_o))

    index_im_all = 0
    time_start = time.time()
    # Iterate over the spike data in slices of size <data_handler.s_slice>
    for ith_slice in range(0, SHAPE_MNIST_TEST[0]//data_handler.s_slice):

        # Retrieve slice <ith_slice> of the spike data
        # spike_data = data_handler.get_mnist_spikes(
        #     ith_slice, tag_mnist=self.tag_mnist, train=False)

        # Ah damn we need the test data spike-ified as well
        ...
        for index_im, spike_times in enumerate(spike_data):
            ...
            # Print, plot, and save data according to the given parameters
            time_start = network.print_plot_save(
                data_handler, X_test, labels_test, index_im_all, SHAPE_MNIST_TEST[0], P, time_start)
