...
class Parameters():
    ...
    def set_common_custom(self, params_custom, argv):
        ...
        self.pf_mnist = self.pd_data + \
            params_custom['nf_mnist']  # Path to MNIST file
        self.pdt_mnist_spikes_train = self.pd_data + \
            params_custom['ndt_mnist_spikes_train']  # Path template to MNIST spikes directory
        self.pdt_mnist_spikes_test = self.pd_data + \
            params_custom['ndt_mnist_spikes_test']  # Path template to MNIST spikes directory
        # Filename template of MNIST spike files
        self.nft_mnist_spikes = params_custom['nft_mnist_spikes']
        # Tag that identifies normally ordered MNIST data
        self.tag_mnist_normal = params_custom['tag_mnist_normal']
        # Tag that identifies shuffled MNIST data
        self.tag_mnist_shuffled = params_custom['tag_mnist_shuffled']
        # Tag that identifies noisy MNIST
        self.tag_mnist_noisy = params_custom['tag_mnist_noisy']
        self.tag_mnist = self.tag_mnist_normal if is_none(
            params_custom['tag_mnist']) else params_custom['tag_mnist']
        ...
        self.load_essential_params_only = self.interpret_boolean(
            params_custom['load_essential_params_only'])
        ...
        self.save_weight_ims_interval = int(
            params_custom['save_weight_ims_interval'])
        self.save_weights_interval = int(
            params_custom['save_weights_interval'])
        self.evaluation_interval = int(params_custom['evaluation_interval'])

        # Parameters for the spike data
        # The firing frequency of each input spike train
        self.hertz_i = float(params_custom['hertz_i'])
        # The duration in miliseconds of each input spike train
        self.ms = int(params_custom['ms'])
        # The size of each slice of data
        self.s_slice = int(params_custom['s_slice'])
        ...
        self.delta_w_max = None if is_none(
            params_custom['delta_w_max']) else float(params_custom['delta_w_max'])

        self.always_fire = self.interpret_boolean(params_custom['always_fire'])

        self.seed = int(params_custom['seed'])

        self.inh_start_o = None if is_none(
            params_custom['inh_start_o']) else float(params_custom['inh_start_o'])
        self.inh_change_o = float(params_custom['inh_change_o'])
        self.inh_ratio_o = float(params_custom['inh_ratio_o'])

        self.e_min_o = None if is_none(
            params_custom['e_min_o']) else float(params_custom['e_min_o'])

        self.eta_mult = float(params_custom['eta_mult'])

        self.softmax_probabilities = self.interpret_boolean(
            params_custom['softmax_probabilities'])
        self.allow_concurrent_spikes = self.interpret_boolean(
            params_custom['allow_concurrent_spikes'])

        self.topdown_enabled = self.interpret_boolean(
            params_custom['topdown_enabled'])
        self.topdown_stdp_mult = float(params_custom['topdown_stdp_mult'])
        self.params_td_curve = None if is_none(
            params_custom['params_td_curve']) else self.interpret_tuple(params_custom['params_td_curve'])
    ...
    def set_single(self, argv):
        ...
        params_custom = self.customize_parameters(
            self.params_default.copy(), argv)
        ...
        # The tag to be used for a given run
        self.tag_mnist = params_custom['tag_mnist']
        ...
        self.print_results = self.interpret_boolean(
            params_custom['print_results'])

        # Whether network weights (converted to pixels) are plotted
        self.plot_weight_ims = self.interpret_boolean(
            params_custom['plot_weight_ims'])

        # Whether or not pixel representation of the weight and/or the weights themselves should be stored
        self.save_weight_ims = self.interpret_boolean(
            params_custom['save_weight_ims'])
        self.save_weights = self.interpret_boolean(
            params_custom['save_weights'])
        ...
        if self.save_weight_ims or self.save_weights or self.save_test_results:
            ...
            self.run_id = len([name for name in os.listdir(self.pd_results) if os.path.isdir(
                self.pd_results+name) and name[:-5] == ndt_run[:-8]])
            self.pd_run = self.pd_results + ndt_run.format(self.run_id)
            os.mkdir(self.pd_run)
            print("Created directory for storing data from this run: <{}>".format(
                self.pd_run))

            # Create the directory where pixel representations of the weights will be stored
            if self.save_weight_ims:
                self.pd_weight_ims = self.pd_run + \
                    params_custom['nd_weight_ims']
            ...
            self.pd_test_results = self.pd_run + \
                params_custom['nd_test_results']
    ...
    def set_hierarchical(self, argv):
        ...
        # The number of neurons per hidden circuit
        self.K_h = int(params_custom['K_h'])
        # The number of neurons for the output circuit
        self.K_o = int(params_custom['K_o'])
        ...
        self.inh_start_h = None if is_none(
            params_custom['inh_start_h']) else float(params_custom['inh_start_h'])
        self.inh_change_h = float(params_custom['inh_change_h'])
        self.inh_ratio_h = float(params_custom['inh_ratio_h'])

        self.e_min_h = None if is_none(
            params_custom['e_min_h']) else float(params_custom['e_min_h'])

        # The target firing frequency of the hidden layer
        self.hertz_h = float(params_custom['hertz_h'])
        ...
        # Whether certain results should be printed
        self.print_results_h = self.interpret_boolean(
            params_custom['print_results_h'])
        self.print_results_o = self.interpret_boolean(
            params_custom['print_results_o'])

        # Whether network weights (converted to pixels) are plotted
        self.plot_weight_ims_h = self.interpret_boolean(
            params_custom['plot_weight_ims_h'])
        self.plot_weight_ims_o = self.interpret_boolean(
            params_custom['plot_weight_ims_o'])

        # Whether or not pixel representation of the weight and/or the weights themselves should be stored
        self.save_weight_ims_h = self.interpret_boolean(
            params_custom['save_weight_ims_h'])
        self.save_weight_ims_o = self.interpret_boolean(
            params_custom['save_weight_ims_o'])
        self.save_weights_h = self.interpret_boolean(
            params_custom['save_weights_h'])
        self.save_weights_o = self.interpret_boolean(
            params_custom['save_weights_o'])
        ...
        if self.save_weight_ims_h or self.save_weight_ims_o or self.save_weights_h or self.save_weights_o:
            ...
            if self.save_weight_ims_h or self.save_weight_ims_o:
                ...
                if self.save_weight_ims_h:  # Create a directory for images of the hidden weights
                    self.pd_weight_ims_h = pd_weight_ims + \
                        params_custom['nd_weight_ims_h']
                ...
                if self.save_weight_ims_o:  # Create a directory for images of the output weights
                    self.pd_weight_ims_o = pd_weight_ims + \
                        params_custom['nd_weight_ims_o']
            ...
            if self.save_weights_h or self.save_weights_o:
                ...
                # Create the upper directory for the weights
                os.mkdir(pd_weights)
                if self.save_weights_h:  # Create a directory for the hidden weights
                    self.pd_weights_h = pd_weights + \
                        params_custom['nd_weights_h']
                ...
                if self.save_weights_o:  # Create a directory for the output weights
                    self.pd_weights_o = pd_weights + \
                        params_custom['nd_weights_o']
