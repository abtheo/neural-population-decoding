...
class WTALayer():
    ...
    def __init__(self, n_rows, n_cols, K, ms):
        ...
        self.excitation = np.zeros(
            (self.n_rows, self.n_cols, self.K), dtype=np.float32)

        # Track the last time each neuron spiked (for simple STDP)
        self.t_last_spike = np.full(
            (self.n_rows, self.n_cols, self.K), -99999, dtype=np.int32)
        ...
        self.t_spikes_pre = np.zeros(
            (self.n_rows, self.n_cols, self.K, self.ms))

        # Track the total amount of times each neuron spiked
        self.n_spikes_total = np.zeros(
            (self.n_rows, self.n_cols, self.K), dtype=np.uint32)
...
class Network():
    ...
    def __init__(self, P):
        ...
        if self.stdp_simple:
            # The time delay during which pre-synaptic spikes are remembered (for simple STDP)
            self.sigma = P.sigma
            ...
            self.stdp_curve = 1/(P.t_f - P.t_s) * (np.exp(-t_diff/P.t_f) -
                                                   np.exp(-t_diff/P.t_s)) * np.heaviside(t_diff, 0)
        # Lower <eta> to <1 / n_spikes_k^eta_root_decay>
        self.eta_root_decay = P.eta_root_decay
        ...
        # A linear shift that brings weights to the range [0, -np.log(w_min_exp)]
        self.log_c = -np.log(P.w_min_exp)
        # The maximum (absolute) value <delta_w> may take
        self.delta_w_max = self.log_c / \
            10 if is_none(P.delta_w_max) else P.delta_w_max
        ...
        self.inhibitor = Inhibitor(
            P.hertz_o, inh_start_o, P.inh_change_o, P.inh_ratio_o, P.ms)
    ...
    def load_weights(self, pf_pretrained_weights):
        ...
        if pf_pretrained_weights != None and pf_pretrained_weights != "None":
            if os.path.exists(pf_pretrained_weights):  # If the path exists ...
                # If the path leads to a directory ...
                if os.path.isdir(pf_pretrained_weights):
                    nfs = os.listdir(pf_pretrained_weights)
                    nfs.sort()
                    weights = [np.load(pf_pretrained_weights+nf)
                               for nf in nfs if os.path.isfile(pf_pretrained_weights+nf)]
                else:  # If the path leads to a file ...
                    weights = np.load(pf_pretrained_weights)
                return weights
            else:
                print("File <{}> does not exist, could not load weights, returning ...".format(
    ...
    def tile_input(self, spike_times, s_os, w, h):
        """ Convert the flat <spike_times> to a tiled array. """
        tiled_input = np.empty(
            (s_os, s_os, w, h, 2, spike_times.shape[-1]), dtype=np.bool8)
        for r in range(s_os):
            for c in range(s_os):
                tiled_input[r, c] = spike_times[r*w:r*w+w, c*h:c*h+h]
        return tiled_input
    ...
    def stdp(self, layer_a, layer_b, weights, t, k, stdp_mult=1):
        ...
        eta = self.eta_mult / \
            np.power(layer_b.n_spikes_total[0, 0, k], self.eta_root_decay)
        ...
        if self.stdp_simple:  # Apply the simple STDP rule
            ...
            delta_w = np.clip((ys*np.exp(-(weights)) - 1*stdp_mult), -
                              self.delta_w_max/eta, self.delta_w_max/eta)
            weights += eta * delta_w
    ...
    def get_spike_indices(self, excitation, K, hz):
        ...
        if self.softmax_probabilities:  # Use global information to compute softmax probabilities based on membrane potentials
            ...
            if self.rng.random() < hz/1000:  # Fire a single neuron
                p = np.exp(excitation-np.max(excitation)) / \
                    np.exp(excitation-np.max(excitation)).sum()
                ks = np.array(
                    [np.random.choice(np.arange(K), p=p)], dtype=np.uint16)
            ...
            if not self.allow_concurrent_spikes:  # Allow but a single neuron to spike at a given timestep
                ks = np.array([], dtype=np.uint16) if ks.size == 0 else np.array(
                    [np.random.choice(ks)], dtype=np.uint16)
    ...
    def propagate_layer(self, layer_a, layer_b, layer_c, weights_ab, weights_bc,
        ...
        for k in ks:
            ...
            if not isinstance(layer_c, type(None)):  # Propagate to the next layer
                ...
                # *** Clipping like this probably does not work with a reference @@@
                layer_c.excitation = np.clip(
                    layer_c.excitation, 0, -self.e_min_o)
    ...
    def plot_weights(self, weight_pixels, n_spikes_since_reset, indices, index_im, input_images, label, alpha_diff=0.5):
        ...
        def set_ax(pixels, ax, alpha, title):
            if np.max(pixels) > 1:  # ***
                ax.imshow(pixels, cmap=plt.get_cmap('gray'),
                          alpha=alpha, vmin=0, vmax=255)
            else:
                ax.imshow(pixels, cmap=plt.get_cmap(
                    'gray'), alpha=alpha, vmin=0, vmax=1)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_title(title, fontsize=10)

        # Determine the proportion that each neuron fired for the last image, used to highlight said neurons
        spike_proportions = np.array(
            n_spikes_since_reset) / max(1, n_spikes_since_reset.sum())
        ...
        fig, axes = plt.subplots(
            nrows=n_rows, ncols=n_cols, figsize=(25, 6 + 2*n_indices//10))
        if n_rows == 1:  # Add an extra dimension if necessary
            axes = np.expand_dims(axes, 0)
        for i, input_image in enumerate(input_images):  # Plot the input images
            set_ax(input_image, ax=axes[0, i], alpha=1,
                   title="Input digit (label={})".format(label))
...
# =============================================================================
        ...
        label_entropies = [[] for _ in range(classes)]
        ...
        label_spike_total = [np.sum(label_spike_counts[i])
                             for i in range(classes)]
        label_spike_avg = [np.mean(label_spike_counts[i])
                           for i in range(classes)]
        entropies_avg = [np.mean(label_entropies[i]) for i in range(classes)]
        entropies_std = [np.std(label_entropies[i]) for i in range(classes)]
        cm_confidence_error = normalize(
        ...
        results_string += "Achieved an accuracy of {:.2f}%".format(
            accuracy) + "\n"
        results_string += "Spike count total: " + \
            str(["{}={:8.2f}".format(i, label_spike_total[i])
                for i in range(classes)]) + "\n"
        results_string += "Spike count means: " + \
            str(["{}={:8.2f}".format(i, label_spike_avg[i])
                for i in range(classes)]) + "\n"
        results_string += "    Entropy means: " + \
            str(["{}={:8.2f}".format(i, entropies_avg[i])
                for i in range(classes)]) + "\n"
        results_string += "    Entropy stdvs: " + \
            str(["{}={:8.2f}".format(i, entropies_std[i])
                for i in range(classes)]) + "\n"
        results_string += "  Mean overall spike count: {:5.2f}".format(
            np.mean(label_spike_avg)) + "\n"
        results_string += "      Mean overall entropy: {:5.2f}".format(
            np.mean(entropies_avg)) + "\n"
        results_string += "      Stdv overall entropy: {:5.2f}".format(
            np.mean(entropies_std)) + "\n"
        results_string += "Mean confidence deviation: {:5.2f}".format(
            np.mean(np.abs(cm_confidence_error))) + "\n"
        results_string += "Stdv confidence deviation: {:5.2f}".format(
            np.std(cm_confidence_error)) + "\n"
        ...
        if plot_cms:
            ...
            util.plot_confusion_matrix(normalize(cm_classifications, axis=0),
                                       xticks=[
                                           digit for digit in range(classes)],
                                       yticks=[
                                           digit for digit in range(classes)],
            ...
            util.plot_confusion_matrix(normalize(cm_spike_confidence, axis=0),
                                       ...
                                       vmin=0, vmax=100, figsize=(10, 7))
            util.plot_confusion_matrix(cm_confidence_error, xticks=[digit for digit in range(classes)],
                                       yticks=[
                                           digit for digit in range(classes)],
