...
class DataHandler():
    ...
    def shuffle_mnist(self):
        ...
        if os.path.isfile(pf_mnist_shuffled):
            print(
                f"File <{pf_mnist_shuffled}> already exists, not creating new shuffled MNIST.")
            return

        # Load the normally ordered MNIST data and create new arrays of the same shape
        X_train, labels_train, X_test, labels_test = self.load_mnist(
            self.tag_mnist_normal)
        X_train_shuffled, X_test_shuffled = np.zeros_like(
        ...
        for digit in range(10):  # Iterate over all ten digits
            ...
            X_train_shuffled[i_train] = X_train[np.where(
                labels_train == digit)[0]]
    ...
    def store_mnist_as_spikes(self, tag_mnist, train=True):
        ...
        if train:  # Store as training data
            pd_mnist_spikes = self.pdt_mnist_spikes_train.format(
                tag_mnist, self.hertz_i, self.ms, self.s_slice, self.path_affix)
        else:     # Store as test data
            pd_mnist_spikes = self.pdt_mnist_spikes_test.format(
                tag_mnist, self.hertz_i, self.ms, self.s_slice, self.path_affix)

        # Create a directory to store the data
        if os.path.isdir(pd_mnist_spikes):  # Only proceed if the directory does not exist
            print("Data directory <{}> already exists, returning ...".format(
                pd_mnist_spikes))
            # print("NOT IN FACT RETURNING BECAUSE TESTING!!!")
            return
        else:
            os.mkdir(pd_mnist_spikes)
            print("Created directory <{}> for storing spike data.".format(
                pd_mnist_spikes))
        ...
        for i in range(0, X.shape[0], self.s_slice):
            ...
            pf_mnist_spikes = pd_mnist_spikes + self.nft_mnist_spikes.format(
                tag_mnist, self.hertz_i, self.ms, slice_start, slice_end, self.path_affix)
    ...
    def create_noisy_mnist(self, noise_percentage, tag_mnist):
        ...
        invert_X_train = self.rng.choice([True, False], size=X_train.shape, p=[
                                         noise_percentage/100, (100-noise_percentage)/100])
        X_train[invert_X_train] = (X_train[invert_X_train]+1) % 2

        labels_train = self.binarize_pixels(labels_train)
        invert_labels_train = self.rng.choice([True, False], size=labels_train.shape, p=[
                                              noise_percentage/100, (100-noise_percentage)/100])
        labels_train[invert_labels_train] = (
            labels_train[invert_labels_train]+1) % 2

        X_test = self.binarize_pixels(X_test)
        invert_X_test = self.rng.choice([True, False], size=X_test.shape, p=[
                                        noise_percentage/100, (100-noise_percentage)/100])
        X_test[invert_X_test] = (X_test[invert_X_test]+1) % 2

        labels_test = self.binarize_pixels(labels_test)
        invert_labels_test = self.rng.choice([True, False], size=labels_test.shape, p=[
                                             noise_percentage/100, (100-noise_percentage)/100])
        labels_test[invert_labels_test] = (
            labels_test[invert_labels_test]+1) % 2

        noisy_mnist = X_train, labels_train, X_test, labels_test
        pf_noisy_mnist = self.pf_mnist.format(
            tag_mnist+self.tag_mnist_noisy.format(noise_percentage))
    ...
    def get_mnist_spikes(self, ith_slice, tag_mnist, train=True):
        ...
        if not os.path.isdir(pd_mnist_spikes):
            print("Data directory <{}> does not exist, returning ...".format(
                pd_mnist_spikes))
            return
